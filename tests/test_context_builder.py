...
def _partial_current_season_tables(**overrides):
    """A player with only 1-2 games this season - last season is still relevant context."""
    player_stats = pl.DataFrame({
    ...
    injuries = pl.DataFrame({"player_name": [], "status": [], "practice_level": []})
    return {"player_stats": player_stats, "projections": projections, "injuries": injuries}
...
def test_build_context_omits_last_season_once_three_current_season_games_exist():
    ...
    context = build_context(["Jordan Love"], week=5, tables=tables)
...
def test_build_context_omits_last_season_line_when_player_has_none(monkeypatch):
    """A player with no prior-season row (e.g. a rookie) skips the "Last season" bullet
    but still shows their single most recent game.
    """
