...
- **Phase 1 — Structured data ingestion: done** (`scripts/refresh_stats.py`; `python scripts/refresh_stats.py` refreshes `data/{raw,staged,processed}/*.parquet` for the **upcoming** week — `resolve_target_week()` picks the earliest week with an unplayed game, stats aggregate over weeks before it, projections/injuries are fetched for it; `tests/test_refresh_stats.py` covers the aggregation/overlay logic. **Fixed 2026-09-19**: `build_processed_player_stats()`'s "last 3 weeks" used to be the league's 3 most-recent calendar weeks intersected per player (not that player's own last 3 *played* games), which silently blended this-season and last-season games into one number and could average as few as 1-2 real games while calling it a "3-week trend" (surfaced by a user report that Chris Olave vs. Malik Nabers looked wrong in week 2). Now: `avg_fantasy_points_last3`/`avg_fantasy_points_season` are this-season-only (each player's own games, never crossing the season boundary); `prior_season_games_played`, `prior_season_avg_fantasy_points`, `prior_season_last3_avg_fantasy_points` surface last season separately; `last_game_*` fields carry the single most recent game played (whichever season) with when it happened disclosed. Three follow-up fixes from PR #11's Codex review, same day: (1) all of the above now use the `_ppr` variants (`avg_fantasy_points_ppr_last3`, etc.) since `projected_points` is PPR-based — the non-PPR fields exist too but must not feed context building; (2) `stats_seasons()` now always fetches `[season - 1, season]` rather than only in the first `LAST_N_WEEKS` weeks, since the prior-season fallback is needed whenever a player's current-season sample is thin, not just early in the season; (3) `build_processed_player_stats()` takes a `current_player_ids` argument (sourced from the current week's staged projections) and filters its output to players with a current-season game or a current roster/projection spot — otherwise (1)+(2) would resurface retirees and unsigned free agents on the strength of stale prior-season production alone)
- **Phase 2 — Context builder: done** (`pipeline/entity_extraction.py`, `pipeline/context_builder.py`; `build_context(players, week)` returns the §7 format minus the "Recent news" bullet, deferred to Phase 6; `tests/test_context_builder.py` covers extraction + assembly. **Updated 2026-09-19**: `_format_recent_form()` never blends seasons — 3+ this-season games shows only this-season lines; fewer shows this-season (or "no games played yet") plus a separate "Last season" line and the single most recent game played, each clearly labeled with its own game count; last season drops out entirely once there are 3+ this-season games)
...
- **Phase 5 — React frontend: done, superseded by the chat assistant below** (`frontend/` Vite + React app; the original player-picker form was replaced 2026-09 by a chat UI — see the chat assistant line. **Remaining post-deployment backlog** (user request, 2026-08-06): defense/DST comparison support, a league scoring config (e.g. reception points), and personal branding/links — see README §10; typeahead search was superseded by `@`-mention search)
